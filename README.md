...
1. **Google login (live site only)**
   - Set `NEXT_PUBLIC_SITE_URL` to the real site (`https://your-app.vercel.app`). Do not use localhost. On Vercel this can fall back to `VERCEL_PROJECT_PRODUCTION_URL`.
   - Supabase (hosted project) → Authentication → Sign In / Providers → Google: enable, paste the Google Cloud **Web application** client ID and secret.
   - Google Cloud authorized redirect URI: `https://<project-ref>.supabase.co/auth/v1/callback` (copy from the Supabase Google provider screen). Authorized JavaScript origins: the live site and `https://<project-ref>.supabase.co`.
   - Supabase → Authentication → URL Configuration: Site URL = `https://your-app.vercel.app`. Redirect URLs: `https://your-app.vercel.app/auth/callback` and `https://your-app.vercel.app/**`.
   - Continue with Google sends the browser to `{SITE}/auth/google`, then Gmail, then `{SITE}/auth/callback`, then **dashboard** (admins: `/admin`). Email/password login is unchanged.
...
2. Apply `supabase/realtime.sql` (adds `notifications` + `appointments` to the `supabase_realtime` publication and sets replica identity).

The browser subscribes with the signed-in user's JWT, so RLS ensures a customer only receives their own notification/appointment events. If Realtime is disabled, the UI falls back to a slow, visibility-aware poll. Local dev's two-database split (Docker Postgres for data, Supabase only for Auth) means Realtime cannot see app writes locally unless you point `DATABASE_URL` at the local Supabase Postgres (port 54322).
